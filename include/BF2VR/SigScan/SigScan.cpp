...
{
	MODULEINFO hModInfo;
	GetModuleInformation(GetCurrentProcess(), GetModuleHandleA(module), &hModInfo, sizeof(hModInfo));
	return hModInfo.SizeOfImage;
}

void *PatternScanner::FindPattern(const std::string &ModuleName, const Pattern &pattern)
{
	return PatternScanner::FindPattern(GetModuleHandleA(ModuleName.c_str()), GetModuleSize(ModuleName.c_str()), pattern);
}
...
{
	return PatternScanner::FindPatterns(GetModuleHandleA(ModuleName.c_str()), GetModuleSize(ModuleName.c_str()), pattern);
}
...
{
	return PatternScanner::FindPattern(GetModuleHandleA(ModuleName.c_str()), GetModuleSize(ModuleName.c_str()), patterns, ratio);
}