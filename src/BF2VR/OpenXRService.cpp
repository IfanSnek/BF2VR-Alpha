...
namespace BF2VR {
	bool OpenXRService::createXRInstanceWithExtensions() {
		...
		xr = xrGetInstanceProcAddr(xrInstance, "xrGetD3D11GraphicsRequirementsKHR", reinterpret_cast<PFN_xrVoidFunction*>(&xrExtGetD3D11GraphicsRequirements));
		if (xr != XR_SUCCESS) {
		...
		// Configure HMD info and blend type
	...
	bool OpenXRService::beginXRSession() {
		...
		graphicsBinding.device = DirectXService::pDevice;
		...
		if (xr != XR_SUCCESS) {
			error("Failed to create OpenXR session. Error: " + std::to_string(xr));
			return false;
		}
	...
	bool OpenXRService::beginFrame()
	{
		...
		// Wait on frame to be ready for rendering
	...
	bool OpenXRService::updatePoses() {
		...
		if (isFiring)
		{
			// If the trigger is down, quickly point the gun at its direction. This is a workaround because the HUD relies on the aiming position.
			if (!HEADAIM)
			{
				hudQuat.w = hq0;
				hudQuat.x = hq1;
